...
class Model(nn.Module):
    def __init__(self, args, word_dict):
        ...
        self.embedding = nn.Embedding(len(word_dict), 300, padding_idx=0)
    ...
    def forward(self, batch):
        ...
        if 'evidence' in g.edata:
            ...
            batch_size, sent_len, word_len = g.edata['evidence'].size()
            all_evidences = g.edata['evidence'].view(batch_size * sent_len, word_len)
            g.edata['evidence'] = self.embedding(all_evidences).view(batch_size, sent_len, word_len, -1)


        ## question representation

        g.ndata['question'] = self.embedding(g.ndata['question'])
        
        q_len = g.ndata['question_mask'].eq(0).sum(1).cpu().numpy().tolist()
        ...
        g.ndata['first_sent'] = self.embedding(g.ndata['first_sent'])
        ...
        sent_rep = self.first_sent_attn(sent_rep, xsent_mask)
        g.ndata['h'] = sent_rep
