...
def vectorize_triviaqa(ex, model, istrain, max_seq_length=64):
    q_id = ex['id']
    text = ex['text']
    ...
    for sup_q in ex['q_et']:
        ...
        input_ids, input_mask = text_tokenize(sub_question, model.word_dict, max_seq_length)
        question_tokens.append(input_ids)
        question_masks.append(input_mask)
...
class TriviaQADataset(DELFTDataset):
    def __init__(self, examples, model, istrain):
        super(TriviaQADataset, self).__init__(examples, model, istrain)


    def __getitem__(self, index):
        return vectorize_triviaqa(self.examples[index], self.model, self.istrain)    
