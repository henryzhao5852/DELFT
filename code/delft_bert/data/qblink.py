...
def vectorize_qblink(ex, tokenizer, device, istrain, max_seq_length=64):
    bert_model.eval()
    t_id = ex['id']
    text = ex['text']
...
class QBLINKDataset(DELFTDataset):
    def __init__(self, examples, args, tokenizer, istrain):
        super(QBLINKDataset, self).__init__(examples, args, tokenizer, istrain)


    def __getitem__(self, index):
        return vectorize_qblink(self.examples[index], self.tokenizer, self.args.device, self.istrain)    
